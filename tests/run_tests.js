...
function req(method, path, body) {
  return new Promise((resolve, reject) => {
    ...
    r.on('error', e => reject(new Error('Server not reachable: ' + e.message)));
...
async function run() {
  ...
  console.log('\n============================================================');
  console.log(`  PASSED: ${pass}/${tests.length}   FAILED: ${fail}/${tests.length}`);
  console.log('============================================================\n');
...
test('GET /api/routes returns all routes', async () => {
  ...
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (!Array.isArray(r.body.routes) || r.body.routes.length === 0) throw new Error('No routes');
});
...
test('GET /api/popular returns enriched popular routes', async () => {
  ...
  if (pr && pr.minPrice === undefined) throw new Error('Missing minPrice in popular route');
});
...
test('GET /api/search weekend date multiplier increases prices', async () => {
  ...
  const rSat = await req('GET', `/api/search?from=Mumbai&to=Pune&date=${satStr}`);
  const rWed = await req('GET', `/api/search?from=Mumbai&to=Pune&date=${wedStr}`);
  if (rSat.status !== 200 || rWed.status !== 200) throw new Error('Search failed');
...
test('GET /api/search with busType filter', async () => {
  const r = await req('GET', '/api/search?from=Mumbai&to=Pune&busType=ac-sleeper');
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  for (const bus of r.body.results) {
    if (bus.busTypeCode && bus.busTypeCode !== 'ac-sleeper') {
      throw new Error('Filter not applied: found busTypeCode=' + bus.busTypeCode);
    }
  }
});
...
test('GET /api/price-trends?from=Mumbai&to=Pune returns history', async () => {
  ...
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (!r.body.history || r.body.history.length !== 7) throw new Error('Expected 7-day history');
...
test('POST /api/coupons/validate valid coupon BUSCOMP100', async () => {
  const r = await req('POST', '/api/coupons/validate', { code: 'BUSCOMP100', amount: 700 });
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (!r.body.valid) throw new Error('Expected valid=true');
  if (r.body.discount !== 100) throw new Error('Expected discount=100, got ' + r.body.discount);
  if (r.body.finalAmount !== 600) throw new Error('Expected finalAmount=600, got ' + r.body.finalAmount);
});

test('POST /api/coupons/validate invalid code returns 404', async () => {
  const r = await req('POST', '/api/coupons/validate', { code: 'FAKEXYZ', amount: 700 });
...
test('POST /api/coupons/validate min spend not met returns 400', async () => {
  const r = await req('POST', '/api/coupons/validate', { code: 'BUSCOMP100', amount: 200 });
  if (r.status !== 400) throw new Error('Expected 400, got ' + r.status);
});

test('POST /api/coupons/validate XSS in code is sanitized', async () => {
  const r = await req('POST', '/api/coupons/validate', { code: '<script>alert(1)</script>', amount: 700 });
...
test('POST /api/admin/login correct password returns token', async () => {
  ...
  if (!r.body.token || r.body.token.length < 10) throw new Error('No valid token');
});
...
test('GET /api/admin/stats has all revenue streams', async () => {
  ...
  const streams = ['busAffiliate', 'flightAffiliate', 'priceLockFees', 'vipSubscriptions', 'sponsoredAds', 'travelInsurance'];
  streams.forEach(s => { if (rb[s] === undefined) throw new Error(`Missing stream: ${s}`); });
});
...
test('POST /api/alert creates price alert successfully', async () => {
  ...
  const r = await req('POST', '/api/alert', {
    email, routeFrom: 'Hyderabad', routeTo: 'Chennai', maxPrice: 900
  });
...
test('POST /api/alert duplicate email+route returns 409', async () => {
  ...
  await req('POST', '/api/alert', { email, routeFrom: 'Mumbai', routeTo: 'Pune' });
  const r2 = await req('POST', '/api/alert', { email, routeFrom: 'Mumbai', routeTo: 'Pune' });
...
test('POST /api/pricelock/create creates a lock', async () => {
  const r = await req('POST', '/api/pricelock/create', {
    busId: 'bus-001', lockedPrice: 799, phone: '9876543210'
  });
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (!r.body.lockId || !r.body.lockId.startsWith('PL-')) throw new Error('Invalid lockId: ' + r.body.lockId);
  if (!r.body.lock.expiresAt) throw new Error('No expiry set');
});

test('POST /api/pricelock/create missing phone returns 400', async () => {
  const r = await req('POST', '/api/pricelock/create', { busId: 'bus-001', lockedPrice: 599 });
  if (r.status !== 400) throw new Error('Expected 400, got ' + r.status);
});
...
test('POST /api/vip/subscribe monthly plan', async () => {
  const r = await req('POST', '/api/vip/subscribe', {
    email: `vip_${Date.now()}@test.com`, phone: '9111111111', plan: 'monthly'
  });
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (!r.body.vipId || !r.body.vipId.startsWith('VIP-')) throw new Error('Invalid VIP ID');
  if (r.body.sub.pricePaid !== 99) throw new Error('Monthly price should be 99');
});

test('POST /api/vip/subscribe annual plan', async () => {
  const r = await req('POST', '/api/vip/subscribe', {
    email: `vipa_${Date.now()}@test.com`, phone: '9222222222', plan: 'annual'
  });
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (r.body.sub.pricePaid !== 299) throw new Error('Annual price should be 299');
});
...
test('GET /api/insurance/quote returns valid quote', async () => {
  ...
  if (r.body.quote.insuranceFee > 149) throw new Error('Fee exceeds cap of 149');
});

test('POST /api/insurance/purchase creates policy', async () => {
  const r = await req('POST', '/api/insurance/purchase', {
    phone: '9000000001', email: 'ins@test.com', busId: 'bus-001', fare: 800, insuranceFee: 65
  });
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (!r.body.policyId || !r.body.policyId.startsWith('INS-')) throw new Error('No policyId');
});

// ─── LOYALTY / BUSCOIN ─────────────────────────────────────────────────────
test('POST /api/loyalty/earn creates new user with coins', async () => {
  const phone = '91' + Math.floor(Math.random() * 1e9).toString().padStart(9, '0');
  const r = await req('POST', '/api/loyalty/earn', { phone, action: 'review' });
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (r.body.coinsEarned !== 15) throw new Error('Review should give 15 coins, got ' + r.body.coinsEarned);
  if (!r.body.referralCode) throw new Error('No referral code');
});

test('GET /api/loyalty/balance returns correct balance', async () => {
  const phone = '88' + Math.floor(Math.random() * 1e9).toString().padStart(9, '0');
...
test('POST /api/referral/generate creates referral code', async () => {
  const r = await req('POST', '/api/referral/generate', { phone: '9555555555' });
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (!r.body.referralCode) throw new Error('No referral code');
...
test('POST /api/reviews/submit creates review', async () => {
  const r = await req('POST', '/api/reviews/submit', {
    busId: 'bus-001', rating: 4, comment: 'Great ride!', travelerName: 'Test User', route: 'Mumbai-Goa'
  });
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
...
test('POST /api/corporate/inquire creates lead', async () => {
  const r = await req('POST', '/api/corporate/inquire', {
    companyName: 'Acme Corp', contactName: 'Raj', email: `corp_${Date.now()}@acme.com`,
    phone: '9000000002', employees: '30', monthlyTrips: '100'
  });
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (!r.body.leadId || !r.body.leadId.startsWith('CORP-')) throw new Error('Invalid leadId');
});

test('POST /api/corporate/inquire missing email returns 400', async () => {
  const r = await req('POST', '/api/corporate/inquire', { companyName: 'NoEmail Inc' });
  if (r.status !== 400) throw new Error('Expected 400, got ' + r.status);
});

test('POST /api/operator/inquire creates operator lead', async () => {
  const r = await req('POST', '/api/operator/inquire', {
    operatorName: 'SpeedBus', contactName: 'Kumar', email: `ops_${Date.now()}@speed.com`,
    phone: '9000000003', fleetSize: '25', routes: 'Chennai-Madurai'
  });
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (!r.body.leadId || !r.body.leadId.startsWith('OPR-')) throw new Error('Invalid leadId');
  // Fleet > 20 should get Enterprise plan
  if (!r.body.message.includes('48 hours')) throw new Error('No onboarding message');
});
...
test('GET /sitemap.xml returns valid XML structure', async () => {
  ...
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
  if (typeof r.body !== 'string' || !r.body.includes('urlset')) throw new Error('Invalid sitemap XML');
});

test('GET /api/admin/leads/export JSON format', async () => {
  const token = await getAdminToken();
  const r = await adminReq('GET', '/api/admin/leads/export?format=json', null, token);
  if (r.status !== 200) throw new Error('HTTP ' + r.status);
...
test('POST /api/review enforces comment length (over 1000 chars)', async () => {
  ...
  const r = await req('POST', '/api/review', {
    busId: 'bus-001', userName: 'Test', rating: 3, comment: longComment
  });
  if (r.status !== 400) throw new Error('Expected 400, got ' + r.status);
});
...
test('POST /api/review enforces name length (over 50 chars)', async () => {
  const r = await req('POST', '/api/review', {
    busId: 'bus-001', userName: 'A'.repeat(51), rating: 4, comment: 'Nice ride'
  });
  if (r.status !== 400) throw new Error('Expected 400, got ' + r.status);
});
...
test('POST /api/review rating is clamped to 1-5', async () => {
  const r = await req('POST', '/api/review', {
    busId: 'bus-001', userName: 'Tester', rating: 99, comment: 'Testing rating clamp'
  });
  if (r.status === 201 && r.body.review) {
    if (r.body.review.rating > 5) throw new Error('Rating not clamped! Got ' + r.body.review.rating);
  }
});
...
test('GET /api/buses/boarding-points/:busId returns boarding data', async () => {
  ...
  if (search.body.results && search.body.results.length > 0) {
    ...
    if (r.status !== 200) throw new Error('HTTP ' + r.status);
    if (!Array.isArray(r.body.boardingPoints)) throw new Error('No boardingPoints');
    if (!Array.isArray(r.body.droppingPoints)) throw new Error('No droppingPoints');
